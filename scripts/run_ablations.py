...
seeds so a claim never rests on one lucky run.
  ...
  E. Jump cap with local search on. B measures the cap with local search off;
     the configuration the paper proposes runs both. QPSO + LS with the cap
     against QPSO + LS without it.
...
raw run) and two figures:
    ...
    data/ablation_jump_cap_ls.png

    python scripts/run_ablations.py
    python scripts/run_ablations.py --seeds 10 --sizes 20 40 60
"""
...
def _run(task):
    ...
    if config == "exact":
        ...
        sol = r.best_solution
    elif config in ("qpso_ls", "qpso", "qpso_nocap", "qpso_ls_nocap"):
        sol = QPSOVRPOptimizer(
            problem, n_particles=N_PARTICLES, max_iter=max_iter, seed=seed,
            use_local_search=config in ("qpso_ls", "qpso_ls_nocap"),
            max_jump_factor=(math.inf if config.endswith("nocap") else None),
...
def main() -> int:
    ...
    parser.add_argument("--sizes", type=int, nargs="+", default=[20, 40, 60, 80, 100],
                        help="Customer counts for ablations B-E (default: 20 40 60 80 100)")
    ...
    # ---- B + C. ablations ----------------------------------------------------
    print("B-E. Jump-cap and local-search ablations...")
    bc_configs = ["qpso_ls", "qpso", "qpso_nocap", "qpso_ls_nocap", "standard_pso", "pso_ls", "ga"]
    ...
    _gap_plot([r for r in bc_runs if r["config"] in ("qpso_ls", "pso_ls")],
              ...
              "data/ablation_ls_attribution.png")
    _gap_plot([r for r in bc_runs if r["config"] in ("qpso_ls", "qpso_ls_nocap")],
              [("qpso_ls", "QPSO + LS with jump cap", SLOT[0], "o"),
               ("qpso_ls_nocap", "QPSO + LS without jump cap", SLOT[1], "s")],
              args.sizes, f"Jump-cap ablation, local search on ({args.seeds} seeds)",
              "data/ablation_jump_cap_ls.png")
    ...
    md = [
        ...
        f"--max-iter {args.max_iter} --sizes {' '.join(map(str, args.sizes))}`. "
        "Every raw run is in `data/ablation_results.json`. "
    ...
    md += ["## E. Jump-cap ablation with local search on", "",
           "B measures the cap with local search off, but the configuration proposed runs "
           "both, and local search could repair the damage an unbounded jump does, or not. "
           "QPSO + LS with the cap against the same QPSO + LS with the cap removed; same "
           "instances and seeds as B–D. Means over feasible runs only, as in D.", "",
           "| Customers | QPSO + LS, cap | QPSO + LS, no cap | Δ (cap − no cap) |",
           "|---|---|---|---|"]
    for n in args.sizes:
        a = _feasible_stats(bc_runs, "qpso_ls", n)[0]
        b = _feasible_stats(bc_runs, "qpso_ls_nocap", n)[0]
        delta = "—" if a is None or b is None else f"{a - b:+.1f} ({100 * (a - b) / b:+.1f}%)"
        md.append(f"| {n} | {_cell(bc_runs, 'qpso_ls', n)} | {_cell(bc_runs, 'qpso_ls_nocap', n)} "
                  f"| {delta} |")
    md += ["", "Paired per seed on penalised fitness, cap against no cap:", "",
           "| Customers | Cap better | Tie | Cap worse | Median paired Δ | Both feasible |",
           "|---|---|---|---|---|---|"]
    md += _paired_detail(bc_runs, "qpso_ls", "qpso_ls_nocap", args.sizes)
    md += ["", "Median runtime (s):", "", "| Customers | Cap | No cap |", "|---|---|---|"]
    for n in args.sizes:
        rt = [statistics.median(r["runtime_s"] for r in bc_runs if r["n"] == n and r["config"] == c)
              for c in ("qpso_ls", "qpso_ls_nocap")]
        md.append(f"| {n} | {rt[0]:.1f} | {rt[1]:.1f} |")
    md += ["", "Figure: `ablation_jump_cap_ls.png`.", ""]
